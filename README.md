...
```bash
# --- Auf deinem Laptop ---------------------------------------------------
# 1. Zip auf den Pi kopieren (Pfad/Benutzer/IP anpassen)
scp ~/Downloads/2026-08-21-dd-was-geht-pi-service.zip pi@raspberrypi.local:~/
...
# --- Ab hier auf dem Pi --------------------------------------------------
# 3. Entpacken
unzip ~/2026-08-21-dd-was-geht-pi-service.zip -d ~/
cd ~/dd-was-geht

# 4. .env aus der Vorlage anlegen und ausfüllen
cp .env.example .env
nano .env      # TELEGRAM_BOT_TOKEN und TELEGRAM_CHAT_ID eintragen, dann Strg+O, Enter, Strg+X

# 5. Bauen und starten (erster Build dauert auf einem Pi 5-15 Minuten)
docker compose up --build -d

# 6. Zusehen, was passiert
docker compose logs -f     # beenden mit Strg+C, der Container läuft weiter
```

Danach: Web-UI unter `http://<pi-ip>:8080`, und in Telegram `/start` an deinen Bot schicken.
...
Wenn du eine neue Version des Pakets bekommst, **nicht** einfach über den Ordner entpacken – das würde `.env` und deine gelernten Bewertungen überschreiben. Stattdessen:

```bash
# Laptop:
scp ~/Downloads/<neues-paket>.zip pi@raspberrypi.local:~/

# Pi:
cd ~/dd-was-geht
./update.sh ~/<neues-paket>.zip
```

`update.sh` sichert vorher `.env` und die Datenbank, spielt nur den Code neu ein und baut den Container neu.

## Später auf Git umsteigen?

Für den Anfang lohnt es sich nicht: Der Code kommt als Zip zu dir, müsste also erst in ein Repo und von dort auf den Pi – das sind mehr Schritte, nicht weniger. Sinnvoll wird Git, sobald du selbst am Code arbeitest oder Versionshistorie und Rollback brauchst. Dann:
...
Die mitgelieferte `.gitignore` hält `.env` (dein Bot-Token!) und die Datenbank bewusst aus der Versionskontrolle heraus. Prüfe nach dem ersten `git add .` einmal mit `git status`, dass `.env` **nicht** in der Liste steht.

Die Web-Oberfläche ist danach unter `http://<pi-ip>:8080` erreichbar. Der Bot antwortet auf `/start`, `/heute`, `/woche`, `/monat`, `/favoriten`.
...
```
Sa 22.08.  18:00  Clueso — Filmnächte am Elbufer · 71%
```
...
Jedes 👍/👎 auf ein Event passt drei Arten von „Gewichten“ an: die **Kategorie** (Musik/Bühne/Familie & Kinder/Touren & Märkte/Sport & Bewegung/Sonstiges), den **Ort** und ein paar **Schlüsselwörter** aus dem Titel. Für jedes neue Event wird daraus ein 0–100-„Für dich“-Score berechnet (Laplace-geglättete Like-Rate über die betroffenen Features, 50 = neutral/unbekannt). Kein Modell, keine externe API, alles nachvollziehbar in `app/scoring.py` (per `python3 tests_smoke.py` durchgetestet). Reaktionen sind idempotent – mehrfaches Klicken derselben Bewertung verändert nichts weiter, ein Wechsel (z.B. von 👎 zu 👍) macht die alte Gewichtung sauber rückgängig.
...
**Das ist zugleich die Rechte-Trennung.** Bewerten kann nur, wer die Flask-Seite im Heimnetz aufmacht – dort landet jedes 👍/👎 in der SQLite und trainiert den „Für dich"-Score. Die öffentliche Kopie zeigt gar keine Bewerten-Buttons mehr: In der statischen Fassung wird der ganze Block per Jinja (`{% if mode != 'static' %}`) aus der Seite herausgerendert, es liegt dort also nicht einmal Code für eine Bewertung. Mitgeliefert wird nur das Ergebnis – das Feld `score` je Event –, damit die Zeile „Empfehlungen der Woche" und der Filter „Wenig relevant" auch öffentlich rechnen können.
...
`tools/export_static.py` rendert **dieselbe** `app/templates/index.html` ein zweites Mal, nur mit `mode="static"` statt `mode="api"` – es gibt also kein zweites Frontend, das mit der Zeit auseinanderläuft. Was sich unterscheidet: Liste und Empfehlungen lesen aus JSON-Dateien statt aus `/api/...`, das Detail-Popup lädt nichts nach, und Bewerten gibt es dort nicht (`CAN_RATE`).
...
**Warum eine Datei pro Tag?** Der Bestand sind rund 5.000 Events auf 32 Tagen ≈ 3 MB. Als eine einzige Datei wäre das bei vier Pushes am Tag jedes Mal ein neuer 3-MB-Blob in der Git-Historie. So schreibt der Export nur die Tage neu, an denen sich wirklich etwas geändert hat (byte-genauer Vergleich vor dem Schreiben), und der Browser lädt für „Heute" auch nur einen Tag (~18 KB gzip). Tage, die aus dem Zeitfenster gelaufen sind, werden gelöscht.
...
- Beide Quellen werden höflich, aber ohne offizielle API abgefragt (1,2 s Pause zwischen Anfragen, klarer User-Agent). Ändert eine Seite ihr Layout grundlegend, ist eine Nachjustierung fällig.
