...
* **Uhrzeit als Gegenprobe.** Mehr als 90 Minuten Abstand (über Mitternacht hinweg gemessen) heißt: verschiedene Termine, auch bei identischem Titel. **Ausnahme bei starker Evidenz:** Stimmen Ort *und* Titel praktisch überein (Wort-Überdeckung ≥ 0,8 **oder** Zeichen-Ähnlichkeit ≥ 0,95), gilt ein Fenster von 150 Minuten – Open-Air-Bühnen nennen mal den Einlass, mal den Beginn. Real am 21.08.2026: Rauze führte „Wincent Weiss“ um 17:00, der Kulturkalender „Wincent Weiss Sommertour 2026“ um 19:00, beide Filmnächte am Elbufer; das Konzert stand dadurch zweimal in der Liste. Bei 150 statt 240 Minuten bleibt eine frühe und eine späte Vorstellung desselben Titels weiterhin getrennt.
...
* **Titel über zwei getrennte Maße.** Die Wort-Überdeckung („stecken alle bedeutsamen Wörter des kürzeren Titels im längeren?“) trägt die Fälle aus der Tabelle oben; die reine Zeichen-Ähnlichkeit taugt dafür nicht (bei „Modus“/„MODUS: Akua“ liegt sie nur bei 0,67). Umgekehrt bleibt die Wort-Überdeckung streng, wenn beide Titel gleich lang sind und sich im entscheidenden Wort unterscheiden: **„MODUS: Akua“ und „MODUS: Anetha“ verschmelzen nicht.** Bei unbekanntem Ort verlangt die Erkennung zusätzlich vollständige Überdeckung von mindestens zwei Wörtern *und* passende Uhrzeiten auf beiden Seiten. **Warum beide Maße auch bei starker Evidenz zählen:** Wörter unter drei Zeichen fliegen aus dem Vergleich, und es gibt Titel, von denen danach nichts übrig bleibt – „S.Y.N.T.H.E.T.I.C S.I.G.N.A.L.S“ hat nach dem Aufbrechen an den Punkten nur einzelne Buchstaben, die Wort-Überdeckung fällt damit zwangsläufig auf 0,0. Der Klubabend im Ostpol stand am 12.09.2026 deshalb zweimal in der Liste (Kulturkalender 20:00, Rauze 22:00), obwohl beide Quellen den Titel zeichengleich schreiben. Seit P5w trägt die Zeichen-Ähnlichkeit denselben Fall.
...
Jedes **Herz** passt drei Arten von „Gewichten“ an: die **Kategorie** (Kultur & Bühne / Musik & Nightlife / Familie & Kinder / Führungen & Touren / Feste & Märkte / Sport & Bewegung / Weiteres), den **Ort** und ein paar **Schlüsselwörter** aus dem Titel. Für jedes neue Event wird daraus ein 0–100-„Für dich“-Score berechnet (Laplace-geglättete Like-Rate über die betroffenen Features, 50 = neutral/unbekannt). Kein Modell, keine externe API, alles nachvollziehbar in `app/scoring.py` (per `python3 tests_smoke.py` durchgetestet).

**Seit P5c ist das Signal einseitig** – das abgelöste 👍/👎 (Tabelle `reactions`, `/api/feedback`, `static/rating.js`) ist vollständig entfallen, Herzen sind sein Ersatz und kein zweites System daneben. Ein Herz zählt **einmal je Reihe**, nicht je Vorstellung: sonst hätte ein Klick auf eine achtmal täglich laufende Dom-Führung achtmal so viel Gewicht wie ein Konzert. Weil es kein Gegenstück zum Daumen-runter mehr gibt, bleibt `weights.skips` dauerhaft 0 und der Score kann 50 nie mehr unterschreiten. Von den drei Verbrauchern des Scores vertragen zwei das (die **Top-Treffer**-Marke ab `HIGHLIGHT_SCORE` und die Sortierung der Empfehlungen); der dritte, der Filter **„Wenig relevant“** (Score < 40), konnte danach nichts mehr ausblenden und ist deshalb entfernt worden statt als toter Schalter stehen zu bleiben.
...
**Herzen und die kuratierte Seite (seit P5c):** an jeder Zeile der Liste sitzt rechts ein ♥. Ein Klick nimmt die Veranstaltung in die eigene Auswahl auf, nachzulesen unter `/herzen` – das ist die **zweite Oberfläche auf derselben Datenbank** und der Grund für den ganzen Umbau: die Liste ist zum Suchen, die Herzen-Seite ist das, was man tatsächlich vorhat. Sie startet leer, es wird dort nichts vorgeschlagen und nichts vorausgewählt.
...
dd-was-geht/
  ...
  app/
    ...
    db.py                  SQLite: Events, Herzen, Gewichte, Scrape-/Backup-Historie
    ...
    templates/herzen.html    Die kuratierte Seite (seit P5c): Davids eigene Auswahl, nur unter Flask
    ...
    static/herzen.js         Herz-Knopf + Entherzen auf der kuratierten Seite (NUR im Heimnetz)
...
- Wird ein Eintrag als Doppelung ausgeblendet, verschwindet er aus der Liste – ein **Herz** auf genau diese Fassung wird beim nächsten Scrape auf den Eintrag gezogen, der sie ersetzt (`db.relink_hearts()`, läuft direkt nach `dedup.link_duplicates()`). Das ist kein Detail: `link_status = 'ok'` heißt „die Ankerzeile ist eine **Gewinner**-Zeile“ (`duplicate_of IS NULL`) und nicht „die Zeile existiert noch“ – eine lebende Zeile mit gesetztem `duplicate_of` ist in jeder Ansicht unsichtbar.
